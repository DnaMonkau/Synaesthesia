...
class SimpleSynaesthesiaNet(nn.Module):
  def __init__(self, input_dim, M, tau=1.0, eta=0.01, eta_w=0.01, cross_talk = True):
    ...
    self.M = M
    self.tolfun = 4e-5
    self.time_step = 0.01 # steps taken in ms
    ...
    if cross_talk:
      ...
      self.K = nn.Parameter(torch.zeros(self.M, self.M) + 1e-4, requires_grad=True) # start at 0 fixed point or near 0
      with torch.no_grad():
  ...
  def learning_rule(self, x, E1, E2):
    ...
    # E2 = - 0.5 * torch.trace(torch.log(chi_2.T @ chi_2))
    if self.FF:
      w_delta1 = - self.eta_w * torch.autograd.grad(E1, self.W1, retain_graph=True)[0]
      w_delta2 = - self.eta_w * torch.autograd.grad(E2, self.W2, retain_graph=True)[0]
      with torch.no_grad():
    ...
    if self.cross_talk:
      ...
      k_delta = - self.eta_k * torch.autograd.grad(torch.mean(torch.stack([E1, E2])), self.K, retain_graph=True)[0]
  ...
  #   return Det
  def convergence(self, s1_prev, s2_prev, s1, s2):
    diff1 = abs(s1 - s1_prev)
    diff2 = abs(s2 - s2_prev)
    difference  = torch.mean(torch.tensor([diff1, diff2])) < self.tolfun
    return difference
  def forward(self, x, max_iter=10):
    ...
    self.x = x
    orig_ts = torch.linspace(0., int((max_iter)/self.time_step), int(max_iter/self.time_step))
    samp_ts = orig_ts
    ...
    for i in tqdm(range(1,int((max_iter/self.time_step)))):
      ...
      if i == 100:
        self.FF = False
      if self.convergence( s1_prev, s2_prev, s1, s2): # steady-stae
        break
      if abs(self.K[0][1]) < 2.0 and abs(self.K[1][0]) < 2.0:
      ...
      self.learning_rule(x, loss_1, loss_2)
...
def make_fig(plots, plots2):
  ...
  # plt.xlim([0, 0.25])
  # plt.ylim([0, 0.25])
  plt.title('Input Variance effects on Cross-talk')
  plt.xlabel('Varianance unit #1')
  plt.ylabel('Varianance unit #2')
  plt.savefig('../Synaesthesia/Shriki27.jpeg')
