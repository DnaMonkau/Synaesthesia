...
%% Iapp and record video
    ...
    dimensions_Iapp = [model.dimensions,size(model.T_Iapp,2)];
    model.Iapp = zeros(dimensions_Iapp);
    model.Iapp_v_full = zeros(model.dimensions_n,'uint8');

%% Prepare model

[model.Iapp, model.T_Iapp, model.T_Iapp_met, model.T_record_met] = ...
    make_experiment(model.images, dimensions);
[model.Post, model.Pre] = create_connections(dimensions);
[model.I_poisson_noise] = make_poisson_noise(dimensions);
end