function [Ca, h, IP3, Iastro_neuron, array_I_neuro] = step_astrocytes(neurons_activity, spike, ...
    array_I_neuro, i, Ca, h, IP3, Iastro_neuron, dimensions)
        ...
        for j = 1 : params.mastro
            for k = 1 : params.nastro
                for l = 1:dimensions(3)
                    if neurons_activity(j, k, l) >= params.min_neurons_activity
                        shift = fix(params.t_neuro / params.step) - 1;
                        array_I_neuro(j, k,l, i : i + shift) = params.amplitude_neuro;
                    end
                end
                %% Compute calcium and IP3 diffusion between neighbors
                if (j == 1) && (k == 1)
                ...
                for l = 1:dimensions(3)
                    if (Ca(j, k, l) > params.threshold_Ca) && (bnh == 0)
                        Fin = any(spike(j, k, l, (i - params.window_astro_watch) : i) >= params.enter_astro);
                        if Fin > 0
                            Iastro_neuron(j, k, l, i : i + params.impact_astro) = 1;
                        end
                    end
                end
            end
